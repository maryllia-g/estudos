...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $nome = $_POST['nome'];
    $cpf = $_POST['cpf']; // Alterado de matricula para cpf
    $dataNascimento = $_POST['dataNascimento'];
    $email = $_POST['email'];
    $usuario_id = $_POST['usuario_id'];

    // Atualiza o paciente no banco de dados
    $stmt = $pdo->prepare("UPDATE pacientes SET nome = ?, cpf = ?, data_nascimento = ?, email = ?, usuario_id = ? WHERE id = ?"); // Alterado de matricula para cpf
    $stmt->execute([$nome, $cpf, $dataNascimento, $email, $usuario_id, $id]);
...
<body>
    ...
    <main>
        <form method="POST">
            ...
            <label for="cpf">CPF:</label> <!-- Alterado de Matrícula para CPF -->
            <input type="text" id="cpf" name="cpf" value="<?= $paciente['cpf'] ?>" required> <!-- Alterado de matricula para cpf -->
